...
/* ── USB 串口芯片库：VID/PID → 开发板（p 省略 = 整个 VID 兜底） ── */
window.COS_DATA.usbdb = [
  { v:0x303A, p:0x1001, nm:'ESP32-S3 / C3（原生 USB）', plat:'esp32', note:'乐鑫原生 USB-CDC，插上即是串口' },
  { v:0x303A, p:0x0002, nm:'ESP32-S2（原生 USB）',      plat:'esp32', note:'乐鑫 USB-OTG' },
  { v:0x303A, p:0x4001, nm:'ESP32 USB-JTAG',            plat:'esp32', note:'乐鑫 JTAG/串口复合设备' },
  { v:0x303A,           nm:'乐鑫 ESP32 系列',            plat:'esp32', note:'Espressif 原生 USB' },
  { v:0x1A86, p:0x55D4, nm:'ESP32 开发板（CH9102）',     plat:'esp32', note:'CH9102 串口，常见于 ESP32-S3 开发板' },
  { v:0x1A86, p:0x7523, nm:'CH340 串口板',              plat:'esp32', note:'CH340：ESP32 / Arduino / 各类开发板通用' },
  { v:0x1A86, p:0x7522, nm:'CH340 串口板',              plat:'esp32', note:'CH340 变体' },
  { v:0x1A86, p:0x5523, nm:'CH341 串口板',              plat:'arduino', note:'CH341 串口' },
  { v:0x1A86,           nm:'沁恒 CH34x 串口设备',        plat:'esp32', note:'CH34x 系列 USB 转串口' },
  { v:0x10C4, p:0xEA60, nm:'CP2102 串口板',             plat:'esp32', note:'CP2102：ESP32 开发板 / SBC 调试口常用' },
  { v:0x10C4, p:0xEA70, nm:'CP2105 双串口',             plat:'rdk',   note:'CP2105 双通道，常见于 SBC 调试口' },
  { v:0x10C4,           nm:'Silabs CP210x 串口',        plat:'esp32', note:'CP210x 系列' },
  { v:0x0403, p:0x6001, nm:'FT232R 串口',               plat:'stm32', note:'FTDI 单通道' },
  { v:0x0403, p:0x6010, nm:'FT2232 双通道（JTAG）',      plat:'stm32', note:'FTDI JTAG + 串口' },
  { v:0x0403,           nm:'FTDI 串口设备',              plat:'stm32', note:'FTDI 系列' },
  { v:0x0483, p:0x3748, nm:'ST-Link V2',                plat:'stm32', note:'STM32 调试器' },
  { v:0x0483, p:0x374B, nm:'ST-Link V2-1 / V3',         plat:'stm32', note:'Nucleo / Disco 板载调试器' },
  { v:0x0483, p:0xDF11, nm:'STM32 DFU（下载模式）',      plat:'stm32', note:'BOOT0 拉高进入 DFU，可直接烧录' },
  { v:0x0483, p:0x5740, nm:'STM32 虚拟串口',            plat:'stm32', note:'STM32 USB CDC' },
  { v:0x0483,           nm:'意法半导体 STM32 设备',      plat:'stm32', note:'STMicroelectronics' },
  { v:0x2341, p:0x0043, nm:'Arduino Uno R3',            plat:'arduino', note:'官方 Uno' },
  { v:0x2341, p:0x8036, nm:'Arduino Leonardo',          plat:'arduino', note:'官方 Leonardo' },
  { v:0x2341,           nm:'Arduino 官方板',             plat:'arduino', note:'Arduino LLC' },
  { v:0x2A03,           nm:'Arduino（.org）板',          plat:'arduino', note:'Arduino SRL' },
  { v:0x2E8A, p:0x0003, nm:'Pico（BOOTSEL 模式）',       plat:'pico', note:'RP2040 U 盘烧录模式' },
  { v:0x2E8A, p:0x0005, nm:'Raspberry Pi Pico',         plat:'pico', note:'RP2040 USB 串口' },
  { v:0x2E8A, p:0x000A, nm:'Pico（MicroPython）',        plat:'pico', note:'RP2040 CDC' },
  { v:0x2E8A,           nm:'Raspberry Pi RP2040 设备',   plat:'pico', note:'树莓派基金会 MCU' },
  { v:0x0D28, p:0x0204, nm:'micro:bit（DAPLink）',       plat:'microbit', note:'拖拽烧录 + 串口' },
  { v:0x0D28,           nm:'mbed / DAPLink 设备',        plat:'microbit', note:'ARM mbed 调试器' },
  { v:0x16C0,           nm:'Teensy',                    plat:'teensy', note:'PJRC Teensy 系列' },
  { v:0x239A,           nm:'Adafruit 开发板',            plat:'xiao',  note:'Adafruit（CircuitPython 常见）' },
  { v:0x2886,           nm:'Seeed XIAO 系列',            plat:'xiao',  note:'矽递 Seeed Studio' },
  { v:0x1366,           nm:'SEGGER J-Link',             plat:'nrf52', note:'nRF52 DK 板载调试器' },
  { v:0x1915,           nm:'Nordic nRF52 设备',          plat:'nrf52', note:'Nordic Semiconductor' },
  { v:0x0955,           nm:'NVIDIA Jetson（恢复/串口）',  plat:'jetson', note:'Jetson USB 设备模式' },
  { v:0x1D6B,           nm:'Linux USB 网卡/串口（板卡）',  plat:'rdk',   note:'SBC 的 USB Gadget：RDK / 树莓派 / Jetson 都可能是它' },
  { v:0x0525,           nm:'Linux Gadget 串口',          plat:'rdk',   note:'SBC 的 USB 串口 Gadget' },
  { v:0x067B,           nm:'PL2303 串口',                plat:'stm32', note:'Prolific 串口' },
  { v:0x1A2C,           nm:'USB 键鼠（非开发板）',        plat:'',      note:'不是开发板' },
];
/* ── 串口启动日志特征：只靠 VID/PID 认不准的板子，读日志二次确认 ── */
window.COS_DATA.logsig = [
  { re:'ESP-ROM:\\s*esp32s3',            nm:'ESP32-S3',       plat:'esp32'  },
  { re:'ESP-ROM:\\s*esp32c3',            nm:'ESP32-C3',       plat:'esp32'  },
  { re:'ESP-ROM:|rst:0x\\d|boot:0x',     nm:'ESP32 系列',      plat:'esp32'  },
  { re:'horizon|hobot|sunrise|rdk ?x',     nm:'RDK X5（地瓜）',  plat:'rdk'    },
  { re:'tegra|jetson|nvidia',              nm:'NVIDIA Jetson',  plat:'jetson' },
  { re:'rk3588|rk3566|rockchip',           nm:'瑞芯微 RK 板',    plat:'rk'     },
  { re:'raspberrypi|raspberry pi|bcm27',   nm:'树莓派',          plat:'pi'     },
  { re:'micropython',                      nm:'MicroPython 板',  plat:'pico'   },
  { re:'stm32|hal_init|cubemx',            nm:'STM32',          plat:'stm32'  },
  { re:'u-boot|linux version',             nm:'Linux 开发板',    plat:'rk'     },
];
